 ...
 *
 * Пути ведут к трём разным источникам, и это видно только отсюда:
 * геометрия движка (ui/props/Models.ts — амфора, костёр, барабан, мелкий
 * камень), рендер из Blender (ART_RUNBOOK.md — у него есть парный файл
 * `-shadow`) и старые картинки Draw Things (всё остальное). Первые два имеют
 * общую камеру и общее солнце; третьи не имеют и переводятся на рендер по
 * одному объекту.
 *
